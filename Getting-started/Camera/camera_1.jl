...
function main()
    ...
    # ------------------------------------
    ourShader = Shader("./shaders/7.1.shader.vs", "./shaders/7.1.shader.fs") # you can name your shader files however you like
    ...
    # set initial time to prevent bugs with sin and cos implementations in julia
    # --------------------------------------------------------------------------
    start_time = time()
    ...
    while !GLFW.WindowShouldClose(window)
        ...
        radius = 10.0f0
        camX = Float32(sin(time_) * radius)
        camZ = Float32(cos(time_) * radius)
        view = lookAt(Vec3(camX, 0.0f0, camZ), Vec3(0.0f0, 0.0f0, 0.0f0),
