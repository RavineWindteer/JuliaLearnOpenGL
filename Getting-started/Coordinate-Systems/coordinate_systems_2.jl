...
function main()
    ...
    # ------------------------------------
    ourShader = Shader("./shaders/6.2.shader.vs", "./shaders/6.2.shader.fs") # you can name your shader files however you like
    ...
    while !GLFW.WindowShouldClose(window)
        ...
        model = Mat4(1.0f0) # make sure to initialize matrix to identity matrix first
        view = Mat4(1.0f0)
        model = rotate(model, time_ * radians(50.0f0), normalize(Vec3(0.5f0, 1.0f0, 0.0f0)))
        view = translate(view, Vec3(0.0f0, 0.0f0, -3.0f0))
        projection = perspective(radians(45.0f0), 
            Float32(SCR_WIDTH)/Float32(SCR_HEIGHT), 0.1f0, 100.0f0)

        # get matrix's uniform location and set matrix
        # note: currently we set the matrix each frame, but since the matrix rarely changes it's often best practice to set it outside the main loop only once.
        use(ourShader)
